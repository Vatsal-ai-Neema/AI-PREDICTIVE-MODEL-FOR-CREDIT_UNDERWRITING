...
# Step 4: Final Decision
    ...
    try:
        ...
        approval_probability = prediction_proba[0][0]
        rejection_probability = prediction_proba[0][1]

        loan_details["approval_probability"] = approval_probability
        loan_details["rejection_probability"] = rejection_probability

        if prediction[0] == 1:
            st.markdown("### Loan Rejected ❌")
            st.error(f"Rejection Probability: {rejection_probability:.2f}")
        else:
            st.markdown("### Loan Approved ✅")
            st.success(f"Approval Probability: {approval_probability:.2f}")

        # Generate PDF with FreeSerif font
        def generate_pdf(loan_details):
            pdf = FPDF()
            # Add a page
            pdf.add_page()

            # Add the FreeSerif font (ensure the FreeSerif.ttf file is in the working directory)
            try:
                pdf.add_font('FreeSerif', '', 'FreeSerif.ttf', uni=True)  # Add custom font
            except Exception as e:
                st.error(f"Error loading font: {e}")
                return None

            # Set font to FreeSerif
            ...
            pdf.cell(200, 10, txt=f"Full Name: {loan_details['full_name']}", ln=True)
            pdf.cell(200, 10, txt=f"Email: {loan_details['email']}", ln=True)
            pdf.cell(200, 10, txt=f"Phone: {loan_details['phone']}", ln=True)
            pdf.ln(10)

            # Loan Details
            pdf.cell(200, 10, txt="Loan Details:", ln=True)
            pdf.cell(200, 10, txt=f"CIBIL Score: {loan_details['cibil_score']}", ln=True)
            pdf.cell(200, 10, txt=f"Loan Amount: ₹{loan_details['loan_amount']}", ln=True)
            pdf.cell(200, 10, txt=f"Loan Term: {loan_details['loan_term']} months", ln=True)
            pdf.cell(200, 10, txt=f"EMI: ₹{loan_details['emi']}", ln=True)
            pdf.ln(10)

            # Prediction Results
            pdf.cell(200, 10, txt="Prediction Results:", ln=True)
            approval_status = "Approved" if loan_details["approval_probability"] > 0.5 else "Rejected"
            pdf.cell(200, 10, txt=f"Prediction: {approval_status}", ln=True)
            pdf.cell(200, 10, txt=f"Approval Probability: {loan_details['approval_probability']:.2f}", ln=True)
            pdf.cell(200, 10, txt=f"Rejection Probability: {loan_details['rejection_probability']:.2f}", ln=True)
        ...
        buffer = generate_pdf(loan_details)

        # Provide download button for the generated PDF
        if buffer:
            st.download_button(
                label="Download Report as PDF",
                data=buffer.getvalue(),  # Use .getvalue() to pass raw bytes to the button
